...
namespace BlackCat\Database\Packages\SyncBatches\Joins;
...
final class SyncBatchesJoins {
    ...
    /**
     * FK: sync_batches -> peer_nodes
     * LEFT JOIN vw_peer_nodes AS $as ON $as.id = $alias.producer_peer_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinPeerNodes(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_peer_nodes AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.producer_peer_id' . ' ', []];
    }
    /**
     * FK: sync_batches -> peer_nodes
     * LEFT JOIN vw_peer_nodes AS $as ON $as.id = $alias.consumer_peer_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinPeerNodesByConsumerPeerId(string $alias = 't', string $as = 'j1'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_peer_nodes AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.consumer_peer_id' . ' ', []];
    }
