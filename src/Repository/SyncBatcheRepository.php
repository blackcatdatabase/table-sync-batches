...
namespace BlackCat\Database\Packages\SyncBatches\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\SyncBatches\Definitions;
use BlackCat\Database\Packages\SyncBatches\Criteria;
use BlackCat\Database\Packages\SyncBatches\Dto\SyncBatcheDto as Dto;
use BlackCat\Database\Packages\SyncBatches\Mapper\SyncBatcheDtoMapper as RowMapper;
...
final class SyncBatcheRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\SyncBatches\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'status', 'items_total', 'items_ok', 'items_failed', 'error', 'started_at', 'finished_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\SyncBatches\Definitions::table());
            return $bulk->upsertMany($rows, [ 'id' ], [ 'status', 'items_total', 'items_ok', 'items_failed', 'error', 'started_at', 'finished_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SyncBatches\Dto\SyncBatcheDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\SyncBatches\Dto\SyncBatcheDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\SyncBatches\Mapper\SyncBatcheDtoMapper::fromRow($row);
    }
