...
namespace BlackCat\Database\Packages\SyncBatches\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\SyncBatches\Dto\SyncBatcheDto;
use BlackCat\Database\Packages\SyncBatches\Mapper\SyncBatcheDtoMapper;
 ...
 */
final class SyncBatchesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SyncBatcheRepository $syncBatcheRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'sync_batches' => $this->syncBatcheRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SyncBatches\ModuleException("Record {$idStr} not found for locking.");
            }
