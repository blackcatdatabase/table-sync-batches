...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 1 outbound / 1 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 1 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\sync-batches](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\sync-batches)
...
graph LR
  ...
  sync_batches["sync_batches"]:::center
  sync_batches -->|FK| peer_nodes["peer_nodes"]:::link
  sync_batch_items["sync_batch_items"]:::inbound -->|FK| sync_batches
...
- Inbound (relies on this): "sync_batch_items"
- Legend: central node = this table, teal/purple arrows = outbound FK targets, green arrows = inbound FK sources.

## Engine Matrix
| Engine | Support |
| --- | --- |
| mysql | ✅ schema(5)<br/>✅ views(3)<br/>⚠️ seeds |
| postgres | ✅ schema(6)<br/>✅ views(3)<br/>⚠️ seeds |
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package sync-batches -Apply
